...
def show_recent_listings(hours: int = 24):
    ...
    with db_manager.get_session() as session:
        ...
        listings = (
            ...
            .filter(ListingModel.scraped_at >= cutoff_date)
            .order_by(ListingModel.scraped_at.desc())
            .limit(20)
            .all()
        )
        ...
        for listing in listings:
            title = (
                listing.title[:47] + "..." if len(listing.title) > 50 else listing.title
            )
            scraped = listing.scraped_at.strftime("%Y-%m-%d %H:%M:%S")
            run_id = (
                listing.run_id[:8] + "..."
                if listing.run_id and len(listing.run_id) > 8
                else listing.run_id or "N/A"
            )
            print(
                f"{title:<50} {listing.price or 'N/A':<15} {scraped:<20} {run_id:<36}"
            )
...
def show_runs_summary():
    ...
    with db_manager.get_session() as session:
        ...
        results = (
            ...
            .filter(ListingModel.run_id.isnot(None))
            .group_by(ListingModel.run_id)
            .order_by(func.min(ListingModel.scraped_at).desc())
            .limit(10)
            .all()
        )
...
def show_database_stats():
    ...
    with db_manager.get_session() as session:
        ...
        latest_listing = (
            session.query(ListingModel).order_by(ListingModel.scraped_at.desc()).first()
        )

        # Get unique run count
        from sqlalchemy import func

        unique_runs = session.query(
            func.count(func.distinct(ListingModel.run_id))
        ).scalar()
...
def show_listings_by_run(run_id: str):
    ...
    with db_manager.get_session() as session:
        listings = (
            ...
            .order_by(ListingModel.scraped_at)
            .all()
        )
...
def main():
    ...
    parser.add_argument(
        "--runs", action="store_true", help="Show summary of scraping runs"
    )
    parser.add_argument("--stats", action="store_true", help="Show database statistics")
    parser.add_argument(
        "--run-id", type=str, help="Show listings for a specific run ID"
    )
