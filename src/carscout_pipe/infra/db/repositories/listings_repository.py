...
class ListingRepository:
    ...
    def get_listings_by_run_id(self, run_id: str) -> List[ListingModel]:
        ...
        return self.session.query(ListingModel).filter(ListingModel.run_id == run_id).all()
    
    def get_all_listings(self, limit: Optional[int] = None) -> List[ListingModel]:
        """Get all listings with optional limit."""
        query = self.session.query(ListingModel).order_by(ListingModel.scraped_at.desc())
        if limit:
    ...
    def count_listings_by_run_id(self, run_id: str) -> int:
        ...
        return self.session.query(ListingModel).filter(ListingModel.run_id == run_id).count()
